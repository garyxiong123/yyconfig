...
public interface AppEnvClusterNamespaceRepository extends JpaRepository<AppEnvClusterNamespace, Long> {
    ...
    @Query(nativeQuery = true, value = "select aecn.*\n" +
            ...
            "where app.app_code=?1 and aec.env=?2 and an.name=?3 and aec.name=?4")
    AppEnvClusterNamespace findAppEnvClusterNamespace(String appCode,String env,String namespace,String cluster);
}
