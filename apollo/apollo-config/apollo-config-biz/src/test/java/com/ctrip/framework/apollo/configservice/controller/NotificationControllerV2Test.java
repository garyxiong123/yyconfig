...
public class NotificationControllerV2Test {
  ...
  private Multimap<String, DeferredResultWrapper> clientWatchedKeyAndDeferredResultWrapperMap;
  ...
  public void setUp() throws Exception {
    ...
    when(bizConfig.releaseMessageNotificationBatchIntervalInMilli()).thenReturn(5l);
    ...
    clientWatchedKeyAndDeferredResultWrapperMap = (Multimap<String, DeferredResultWrapper>) ReflectionTestUtils.getField(controller, "clientWatchedKeyAndDeferredResultWrapperMap");
  }
  ...
  public void testPollNotificationWithMultipleNamespacesAndHandleMessage() throws Exception {
    ...
    controller.handleReleaseMessage(someReleaseMessage, Topics.APOLLO_RELEASE_TOPIC);
  ...
  private void assertWatchKeys(Multimap<String, String> watchKeysMap, DeferredResult deferredResult) {
    for (String watchKey : watchKeysMap.values()) {
      Collection<DeferredResultWrapper> deferredResultWrappers = clientWatchedKeyAndDeferredResultWrapperMap.get(watchKey);
      boolean found = false;
      for (DeferredResultWrapper wrapper: deferredResultWrappers) {
        if (Objects.equals(wrapper.getDeferredResult(), deferredResult)) {
          found = true;
        }
      }
      assertTrue(found);
