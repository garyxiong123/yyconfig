    ...
    if (clusters.length > 1) {
      return (
        <SubMenu title={item.env} key={`${item.env}${index}`}>
          {clusters.map((vo, i) => (
            <Menu.Item key={vo.id} onClick={() => this.onEnvClick(item, vo)}>
              {vo.name}
            </Menu.Item>
          ))}
        </SubMenu>
      ...
      return (
        <Menu.Item key={clusters[0] && clusters[0].id} onClick={() => this.onEnvClick(item)}>
          {item.env}
        </Menu.Item>
      );
    }
      ...
      <Card title="环境列表">
        {currentEnv.env && (
          <Menu
            mode="inline"
            style={{ width: '100%' }}
            selectedKeys={[curentKey]}
            defaultOpenKeys={[currentEnv['env'] + '0']}
          >
    ...
    return (
      <Fragment>
        <Card className={styles.marginTop20} title="操作">
          <Button block type="dashed" onClick={() => this.onShowModal('showClusterModal')}>
            + 添加集群
          </Button>
          <Button
            block
            ...
            type="dashed"
            onClick={() => this.onShowModal('showNamespace')}
          >
            + 添加命名空间
          </Button>
          <Button block className={styles.marginTop20} type="dashed">
            命名空间管理
          </Button>
        </Card>
        {showClusterModal && (
          <ClusterAdd
            onCancel={() => this.onCancel('showClusterModal')}
            onSave={this.onClusterAddSave}
          />
        )}
        {showNamespace && (
          <NamespaceAdd
            onCancel={() => this.onCancel('showNamespace')}
            onSave={this.onNamespaceAddSave}
          />
        )}
      </Fragment>
    );
  ...
  render() {
    ...
    return (
      <PageHeaderWrapper title="项目信息" content={this.renderBaseInfo()} extra={this.renderEdit()}>
        ...
        {showProjectEdit && (
          <CreateProject
            onCancel={() => this.onCancel('showProjectEdit')}
            onSave={this.onSave}
            appId={appId}
          />
        )}
